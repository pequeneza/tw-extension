...
// @version      2.0.0
// @description  Precision attack scheduler: SharedWorker timer, performance.now busy-wait, queue watcher, place-page filler, confirm auto-click.
...
(function () {
  ...
  var cfg = (window.__twSuiteCfg && window.__twSuiteCfg('auto_sender')) || {};
  ...
  function getServerNowMs() {
    try { if (window.Timing && window.Timing.getCurrentServerTime) return window.Timing.getCurrentServerTime(); } catch (e) {}
    return Date.now();
  }
  ...
  function measurePing(multiplier) {
    ...
    var intervalId = null;
    function takeSample() {
      var src = location.origin + '/favicon.ico?_=' + Date.now();
    ...
    takeSample();
    intervalId = setInterval(takeSample, 3000);
    return {
      getOffset: function() {
  ...
  document.addEventListener('xbot:autosender:run', function(e) {
    ...
    else if (d.action === 'applySettings')           { loadSettings(); }
  });
  ...
  function scheduleClickAtMs(targetServerMs, clickFn, getOffsetFn, onFinePhase) {
    ...
    function finePhase() {
      if (stopped) return;
      // Stop any caller-managed intervals before the busy-wait (e.g. countdown tick)
      try { if (onFinePhase) onFinePhase(); } catch (e) {}
      // Freeze all Timing tick handlers (stops game's own intervals, anti-throttle like Kumin)
      try { if (window.Timing && window.Timing.resetTickHandlers) window.Timing.resetTickHandlers(); } catch (e) {}
      // Resolve offset at the moment the 2-s window opens (same as Kumin's getAveragePing() call)
      var offset = (getOffsetFn ? getOffsetFn() : 0);
      var adjustedTarget = targetServerMs + offset;
      // Pure busy-wait for the remaining ~2 s — identical to Kumin's while() loop
      var perfDeadline = performance.now() + (adjustedTarget - getServerNowMs());
      while (!stopped && performance.now() < perfDeadline) {}
      doClick();
    }

    // Fire 2000 ms before target, matching Kumin's "launchTime - serverTime - 2000"
    var coarseWait = Math.max(0, (targetServerMs - getServerNowMs()) - 2000);
    var t = setTimeout(finePhase, coarseWait);

    function onVis() {
      if (!stopped && (targetServerMs - getServerNowMs()) < 2100) { clearTimeout(t); finePhase(); }
    }
  ...
  /* ─── SharedWorker-backed interval (prevents background-tab throttling) ─── */
  var WORKER_SRC =
    'var t={};' +
    'self.onconnect=function(e){' +
      ...
      'p.onmessage=function(m){' +
        'var d=m.data;' +
        'if(d.type==="start"){clearInterval(t[d.id]);t[d.id]=setInterval(function(){p.postMessage({id:d.id});},d.ms);}' +
        'else{clearInterval(t[d.id]);delete t[d.id];}' +
      '};' +
      'p.start();' +
    '};';

  function makeWorkerInterval(fn, ms) {
    try {
      var blob = new Blob([WORKER_SRC], { type: 'application/javascript' });
      var url  = URL.createObjectURL(blob);
      var sw   = new SharedWorker(url);
      sw.port.start();
      sw.port.onmessage = function(e) { if (e.data.id === 'tick') fn(); };
      sw.port.postMessage({ type: 'start', id: 'tick', ms: ms });
      return function() { sw.port.postMessage({ type: 'stop', id: 'tick' }); };
    } catch (err) {
      console.warn('[AutoSender] SharedWorker unavailable, falling back to setInterval:', err.message);
  ...
  function fmtCd(diff) {
    var sign = diff < 0 ? '-' : '';
    var abs  = Math.floor(Math.abs(diff));   // floor first — eliminates float ms artefacts
  ...
  function showConfirmCountdown(cmd) {
    ...
    function render() {
      var diff  = cmd.launch - getServerNowMs();
  ...
  function handlePlacePage() {
    ...
    var isSupport = (cmd.type === 'support');
    setTimeout(function() {
      var btn = isSupport
        ? (document.getElementById('target_support')
            || document.querySelector('input[type="submit"][value*="Apoio"]')
  ...
  function handleConfirmPage() {
    ...
    // Schedule the click at cmd.launch; resolveOffset() provides the final ms adjustment.
    // stopCountdownTick freezes the overlay at the 2-s busy-wait entry (avoids setInterval
    // callbacks accumulating in the queue and interfering with the while() loop).
    scheduleClickAtMs(cmd.launch, function() {
      ...
      if (btn) {
        ...
        emitState();
        showStatus('AutoSender: enviado!' + (_settings.autoClose ? ' A fechar...' : ''), '#15803d');
        try { sessionStorage.removeItem(SS_CONF); } catch (e) {}
  ...
  function runWatcher() {
    function tick() {
      ...
      for (var i = 0; i < q.length; i++) {
        ...
        var until = e.launch - now;
        if (until > (_settings.lookahead || 40) * 1000) continue;
        if (until < -10000)   { e.status = 'failed'; changed = true; continue; }
